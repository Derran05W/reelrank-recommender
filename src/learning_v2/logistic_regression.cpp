...
void LogisticRegression::train(const std::vector<std::vector<double>> &x,
    ...
    // Standardize once up front; SGD then iterates over the standardized matrix in a fixed,
    // seed-determined order (contracts §5: fixed iteration order, per-epoch shuffle).
    ...
    for (int epoch = 0; epoch < hp.epochs; ++epoch) {
        ...
        for (std::size_t start = 0; start < idx.size(); start += batch) {
            ...
            for (std::size_t k = start; k < end; ++k) {
                ...
                const double err = sigmoid(dot) - y[idx[k]]; // d(log-loss)/d(logit)
                for (std::size_t j = 0; j < nFeat; ++j) {
                    gradW[j] += err * zi[j];
                }
                gradB += err;
            ...
            for (std::size_t j = 0; j < nFeat; ++j) {
                // Averaged batch gradient + L2 on weights only (never the bias).
...
double LogisticRegression::predictProba(const std::vector<double> &xRaw) const {
...
nlohmann::json LogisticRegression::toJson() const {
...
LogisticRegression LogisticRegression::fromJson(const nlohmann::json &j) {

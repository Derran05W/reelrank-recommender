...
ExactCandidateSource::ExactCandidateSource(const ExactVectorIndex &index) : index_(index) {}

std::vector<Candidate> ExactCandidateSource::generate(const User &user,
    ...
    for (const VectorSearchResult &result : results) {
        ...
        candidate.source = CandidateSource::VectorExact;

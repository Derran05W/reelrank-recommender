...
//
//   score = wS*S + wQ*Q + wF*F + wP*P + wT*T + wC*C + wE*E + wD*D - wR*R - wI*I
//
// and returns the candidates sorted by rankingScore DESCENDING, ties broken by ascending ReelId
// (a total order, so the output is fully deterministic - same doctrine as the Orchestrator).
//
// Every returned candidate carries its rankingScore AND a featureContributions map with all ten
